    ...
    if x2 <= MAXDIMENTION then
      ...
      begin
        ...
        begin
          ...
          inventory.AddLoosePart((pcs.Objects[k] as TPieceColorInfo).piece, db.Colors(colorindexes[c].idx).id, 1);
          for i := x to x2 - 1 do
            for j := y to y2 - 1 do
...
begin
  ...
  try
    ...
    begin
      ...
      EditCostNew.Text := Format('€ %2.2f', [mosaic.inventory.SoldPartOutValue_nQtyAvg.value]);
      EditCostNew.ReadOnly := True;
      EditCostUsed.ReadOnly := False;
      EditCostUsed.Text := Format('€ %2.2f', [mosaic.inventory.SoldPartOutValue_uQtyAvg.value]);
