...
uses SysUtils, Classes, PNGZLIB1, Math;
...
begin
  ...
  begin
    ...
    begin
      ...
      DStream := TZDecompressionStream.Create(Stream);
      try
        pos := 0;
...
begin
  ...
  { Δεσμεύουμε χώρο στην αρχή του Stream.
    ...
    CStream := TZCompressionStream.Create(Stream, CompressionLevel);
    try
...
function TCustomBinaryData.GetCompressionLevel: TZCompressionLevel;
begin
  if Assigned(FBinary) then
...
procedure TCustomBinaryData.SetCompressionLevel(Value: TZCompressionLevel);
begin
  if Assigned(FBinary) then
