...
const
  C_tiles: array[0..NUMTILES - 1] of mosaicbrick_t = (
    (brick: '3070b'; x: 1; y: 1; use: true),
    (brick: '3069b'; x: 1; y: 2; use: true),
    (brick: '3068b'; x: 2; y: 2; use: true),
    (brick: '63864'; x: 1; y: 3; use: true),
    (brick: '2431';  x: 1; y: 4; use: true),
    (brick: '6636';  x: 1; y: 6; use: true),
    (brick: '4162';  x: 1; y: 8; use: true),
    (brick: '26603'; x: 2; y: 3; use: true),
    (brick: '87079'; x: 2; y: 4; use: true),
    (brick: '69729'; x: 2; y: 6; use: true),
    (brick: '1751';  x: 4; y: 4; use: true),
    (brick: '6881';  x: 6; y: 6; use: true),
    (brick: '90498'; x: 8; y: 16; use: true),
    (brick: '48288'; x: 8; y: 16; use: true)
  );

function SortListByPrice_Tile(List: TStringList; Index1, Index2: Integer): Integer;
begin
  Result := SortListByPrice(@C_tiles, NUMTILES, List, Index1, Index2);
end;
...
{      if pci.EvaluatePriceUsed > 0.10 then
  ...
  for i := 0 to NUMCOLORINDEXES - 1 do
    ...
    begin
      ...
      colorindexes[i].pieces.CustomSort(SortListByPrice_Tile);
    end;
end;
...
begin
  ...
  begin
    ...
    idx := brickindex(@C_tiles, NUMTILES, piece);
    if idx < 0 then
      Continue;
    if not bricks[idx].use then
      Continue;

    dx := brickdimentionx(@C_tiles, NUMTILES, piece);
    dy := brickdimentiony(@C_tiles, NUMTILES, piece);
...
begin
  ...
  try
    ...
    CheckListBox2.Checked[brickindex(@C_tiles, NUMTILES, '3070b')] := true;
