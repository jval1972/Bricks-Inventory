...
type
  ...
  protected
    ...
    property DropAnchor: TDropDownAnchor read FDropAnchor write SetDropAnchor default ddAnchorDown;
    property Action;
  ...
  protected
    ...
    property OnBeforePopup: TAllowPopupNotifyEvent read FOnBeforePopup write FOnBeforePopup;
  end;
...
var DropDownButtonComponents: TStringList;
...
begin
  ...
  begin
    if (FState in [bsDown, bsExclusive]) or
       (FMouseInControl and (FState <> bsDisabled) and MouseInButton and (GetCapture = 0){!!! bad programming (GetCapture=nil)}) or
...
procedure TCustomDropDownButton.MouseDown(Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
var i: integer;
begin
  inherited MouseDown(Button, Shift, X, Y);
  for i := 0 to DropDownButtonComponents.Count - 1 do
    if TCustomDropDownButton(DropDownButtonComponents.Objects[i]) <> self then
      if TCustomDropDownButton(DropDownButtonComponents.Objects[i]).FMouseInControl then
      begin
...
procedure TCustomDropDownButton.MouseUp(Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
var DoClick: Boolean;
begin
...
procedure TCustomDropDownButton.DrawTriangle(Canvas: TCanvas; Top, Left, Width: Integer);
begin
...
procedure TCustomDropDownButton.ActionChange(Sender: TObject; CheckDefaults: Boolean);
...
begin
  ...
  begin
    ...
    if FDropAnchor = ddAnchorDown then
      ...
      p.x := p.x + Width;
    allow := true;
    if Assigned(FOnBeforePopup) then FOnBeforePopup(self, allow);
    if allow then FMenu.Popup(p.x, p.y);
...
procedure TMenuTool.MouseDown(Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
  Inherited MouseDown(Button, Shift, X, Y);
  if Enabled then
  begin
